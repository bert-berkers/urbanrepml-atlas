...
📄 **`report.pdf`** (private repo only, not part of this public atlas) — the full write-up (methods, related work, supervised probes, maps).
This is **Part 2** of the smoothing story; Part 1 is `../2026-07-18-smooth-vs-learn-res10/` (private repo only, not part of this public atlas; read-only, frozen — this campaign consumes its artifacts and never amends it).
...
| Path | What | In this atlas? |
|---|---|---|
| `PREREGISTRATION.md` | The frozen contract: hypotheses, SESOIs, stop rules, full cell grid, filled RESULT/VERDICT slots | Private repo only |
| `report.pdf` | The write-up | Private repo only |
| `latex/` | Report source + build instructions | Private repo only |
| `data/` | `verdicts_2026-07-20.md` (per-cell basis tables), probe-cell summary, stratifier scores + frozen bins, run logs | Private repo only |
| `figures/` | Methodology figures (kernel geometry on real hexagons, effective-kernel profiles, the subtract-the-low-pass story, dose strip, target gallery) | **Partial** — only `fig_kernel_geometry.png` and the flattened `fig_probe_dose_response.png` are copied here |
| `figures/maps_2026-07-20/` | Map battery: 7 arms × 2 views, each national map paired with a South Holland zoom | Private repo only |
